# ---- Purpose ----
# Unpack results of a simulation re-run after JOS ref report

library(EMsigex)
library(sigex)

# Load Sigma estimates
load("~/GitHub/EMsignal/tests/1-B12/results/out-20240304.Rdata")

# Load simulated series
load("~/GitHub/EMsignal/tests/1-B12/ire/dataList.Rdata")

# Align number of estimates and series
nreps = min(length(dataList), length(out))
dataList = dataList[1:nreps]
out = out[1:nreps]
...
# ---- Look at MLE estimation -------------------------------------------------

Sig.mle <- matrix(NA, nrow = nreps, ncol = 27) # storage
for(i in 1:nreps){
...
# ---- look at EM estimation --------------------------------------------------

Sig.em <- matrix(NA, nrow = nreps, ncol = 27) # storage
for(i in 1:nreps){
  # Find last row not na = number of iters before convergence
  max_row_not_na <- which(!is.na(SS), arr.ind = TRUE)[, "row"] |> max()
  Sig.em[i, ] = out[[i]]$Sig.save[max_row_not_na, 1:27]
}
