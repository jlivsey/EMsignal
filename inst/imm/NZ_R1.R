...
# Plot imm with smaller window
ts_span(imm, 2008, 2011) |>
  ts_df() |>
  ...
  facet_wrap(vars(id), ncol = 3, scales = "free") +
  labs(x = NULL, y = NULL)

ggsave(filename = file.path("~/GitHub/EMsignal/inst/imm/", "imm_2008_2011.pdf"),
       plot = last_plot(),
       width = 6,
       height = 4)
...
ggsave(filename = file.path("~/GitHub/EMsignal/inst/imm/", "imm_with_trend.pdf"),
       plot = last_plot(),
       width = 6,
       height = 4)
